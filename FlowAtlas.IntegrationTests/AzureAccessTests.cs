using FlowAtlas.Azure;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.Options;
using System.Text.Json;
using Xunit.Abstractions;

namespace FlowAtlas.IntegrationTests;
...
public class AzureAccessTests
{
    ...
    public AzureAccessTests(ITestOutputHelper output)
    {
        ...
        _opts = config.GetSection(FlowAtlasOptions.Section).Get<FlowAtlasOptions>()
                ?? throw new InvalidOperationException("FlowAtlas config section missing.");
    ...
    private void SkipIfNotConfigured()
    {
        if (string.IsNullOrWhiteSpace(_opts.SubscriptionId) || string.IsNullOrWhiteSpace(_opts.ResourceGroup))
            throw new SkipException(
                "FlowAtlas:SubscriptionId and FlowAtlas:ResourceGroup must be set in " +
                "FlowAtlas.IntegrationTests/appsettings.json to run integration tests.");
    }
